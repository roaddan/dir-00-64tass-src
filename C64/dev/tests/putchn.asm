...
;-------------------------------------------------------------------------------
                ...
                ;lda     putchn_ra       ; Si a contenait un param d'entrée alors! 
;-------------------------------------------------------------------------------
...
;-------------------------------------------------------------------------------
                ...
                tax
                cpx     #0                
                beq     putchn_out      ; au dernier para on sort
putchn_l        jsr     putch           ; on fait la job
putchn_car      .byte   0
                dex                     ; param suivant
                bne     putchn_l        ; on limite la chaine a 255 b
;-------------------------------------------------------------------------------
; Ici le travail est terminé, on recalcule l'adresse de retour pour le RTS ....                
;-------------------------------------------------------------------------------
                ;pha                     ; Si A contient un paramètre de retour.
putchn_out      clc                     ; On ne veux pas ajouter le CARRY
...
;-------------------------------------------------------------------------------
                lda     putchn_carcol   ; On redonne à BASIC sa couleur.
                sta     CARCOL
                lda     putchn_zl       ; et on récupère les registres que nous                
...
;-------------------------------------------------------------------------------
putchn_ra      .byte   0              ; Ces lignes constitues un pseudo stack. 
putchn_rx      .byte   0              ; Même su cette méthode est couteuse en 
putchn_ry      .byte   0              ; Voici
putchn_sf      .byte   0              ; Le stack du 6502 étant limité à une 
putchn_zl      .byte   0              ; seule page de 256 octets et qu'il est
putchn_zh      .byte   0              ; largement utilisé par Basic, cette
putchn_carcol  .byte   0              ; me semble la plus simple a utiliser.
